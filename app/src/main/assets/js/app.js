...
function showReactionBar(x, y, messageId, messageText, isSent) {
    ...
    addActionRow({
        ...
        label: 'رد',
        onClick: () => startReply(messageId, messageText)
    });
...
function setupLogin() {
    ...
    document.getElementById('loginBtn').onclick = async function() {
        ...
        if (window.loginMode === 'signup') {
            // ================================================================
            // قيد "حساب واحد لكل IP" عند إنشاء حساب جديد فقط (تسجيل الدخول
            // لحساب موجود مسبقاً يبقى بلا أي قيد ويُسمح به عدد لا نهائي من
            // المرات - هذا فعلياً سلوك Firebase Auth الطبيعي أصلاً: أي شخص
            // يعرف اسم المستخدم وكلمة المرور الصحيحين يدخل، ومن دونهما لا
            // يقدر أحد الدخول لحساب غيره).
            // ------------------------------------------------------------
            // تنويه مهم وصريح (وليس تفصيلاً تقنياً بحتاً): هذا التحقق "قدر
            // المستطاع" فقط، وليس حماية مضمونة 100%. السبب: هذا كود يعمل
            // بالكامل على جهاز الزائر (WebView)، ولا توجد وسيلة موثوقة من
            // داخل جافاسكربت لمعرفة عنوان IP الحقيقي للجهاز إلا بسؤال خدمة
            // خارجية (ipify هنا) عنه - والعنوان الذي تُرجعه هذه الخدمة نفسه
            // قابل للتغيير بسهولة (VPN، بيانات جوال بدل واي فاي، إعادة تشغيل
            // الراوتر...)، كما أن عدة أشخاص فعليين مختلفين قد يشتركون بنفس
            // IP العام (شبكة منزل واحد، جامعة، شركة اتصالات NAT) فيُمنع
            // شخص شرعي عن الخطأ. الحل الوحيد المضمون فعلاً هو خدمة سيرفر
            // خاصة (Cloud Function) تتحقق من IP الحقيقي لطلب الشبكة نفسه
            // (غير قابل للتزوير من العميل) - وهذا يتطلب نشر كود على مشروع
            // Firebase مباشرة (Cloud Functions)، وهي خطوة خارج نطاق ملفات
            // الواجهة هذه ولا يملك أي مساعد كود إمكانية نشرها تلقائياً على
            // حسابك. اعتبر هذا رادعاً إضافياً معقولاً، وليس بديلاً كاملاً.
            try {
                ...
                try {
                    ...
                    console.warn('⚠️ تعذّر جلب IP الجهاز (لا اتصال بخدمة ipify) - سيُسمح بالتسجيل بدون هذا التحقق هذه المرة');
                }

                if (signupIp) {
                    const ipRef = db.collection('signupIPs').doc(signupIp.replace(/[.:]/g, '_'));
                    const ipDoc = await ipRef.get();
                    const existingCount = ipDoc.exists ? (ipDoc.data().count || 0) : 0;
                    if (existingCount >= MAX_ACCOUNTS_PER_IP) {
                        document.getElementById('loginError').textContent =
                            '✖ يوجد حساب مسجّل مسبقاً من هذا الاتصال — سجّل دخول لحسابك بدل إنشاء حساب جديد';
                        resetBtn();
                        return;
                    }
                    await ipRef.set({
