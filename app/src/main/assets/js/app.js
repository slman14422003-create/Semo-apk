...
// يتحقق من وجود تحديث جديد عند دخول أي مستخدم للدردشة، ويعرض البانر لو وُجد.
// لو التحديث "إجباري" يُعرض دوماً (يتجاهل تفضيل الإغلاق المحفوظ محلياً).
async function checkForAppUpdate() {
    try {
        const doc = await db.collection('app_meta').doc('update_info').get();
        // نحفظ آخر نتيجة معروفة عالمياً (window.__latestUpdateInfo) ونحدّث
        // علامة "*" بجانب "حول" بالقائمة الجانبية دائماً، حتى لو المستخدم
        // كان قد أغلق بانر التحديث سابقاً - البانر يُغلَق، أما هذه العلامة
        // فتبقى تذكيراً هادئاً بوجود تحديث فعلي لحين تثبيته.
        window.__latestUpdateInfo = doc.exists ? doc.data() : null;
        if (!doc.exists) { if (typeof setUpdateBadgeVisible === 'function') setUpdateBadgeVisible(false); return; }
        const data = doc.data();
        const remoteCode = data.versionCode || 0;
        if (typeof setUpdateBadgeVisible === 'function') setUpdateBadgeVisible(remoteCode > window.APP_VERSION_CODE);
        if (remoteCode <= window.APP_VERSION_CODE) return;

        const dismissedCode = parseInt(localStorage.getItem('dismissedUpdateCode') || '0', 10);
        if (!data.mandatory && dismissedCode >= remoteCode) return;

        const banner = document.getElementById('updateBanner');
        if (!banner) return;
        document.getElementById('updateBannerTitle').textContent =
            data.mandatory ? `⚠️ تحديث إجباري - الإصدار ${data.versionName}` : `يتوفر تحديث جديد - الإصدار ${data.versionName}`;
        document.getElementById('updateBannerSub').textContent = data.changelog || '';
        const closeBtn = document.getElementById('updateBannerClose');
        closeBtn.style.display = data.mandatory ? 'none' : '';

        document.getElementById('updateBannerBtn').onclick = function () {
        ...
        window.__latestUpdateCode = remoteCode;
...
function setupUpdateBanner() {
    ...
    if (!closeBtn) return;
    closeBtn.onclick = function () {
        if (window.__latestUpdateCode) {
            localStorage.setItem('dismissedUpdateCode', String(window.__latestUpdateCode));
        }
...
function setUpdateBadgeVisible(visible) {
    ...
    const menuBadge = document.getElementById('menuAboutBadge');
    if (menuBadge) menuBadge.style.display = visible ? 'inline' : 'none';
}
