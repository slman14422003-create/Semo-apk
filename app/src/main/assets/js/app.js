...
window.replyingToUser = '';
window.replyingToText = '';
window.allUsers = {};
window.badWords = [];

// ================================================================
//  إصدار التطبيق الحالي (لنظام التحديثات)
// ================================================================
// إصلاح جذري: كان الإصدار المحلي هنا رقمين ثابتين (hardcoded) يجب تعديلهما
// يدوياً بكل نسخة جديدة، بالتزامن مع versionCode/versionName بملف
// android/app/build.gradle - أي مصدرين منفصلين لنفس المعلومة. لو نُسي
// تحديث هذا الملف عند بناء نسخة APK جديدة (وهو سهل النسيان لأنه خطوة إضافية
// غير قياسية بعملية إصدار أندرويد)، يبقى التطبيق "يظن" نفسه على الإصدار
// القديم حتى بعد تثبيت التحديث فعلياً - فتستمر شاشة التحديثات بإخبار
// المستخدم أن هناك تحديثاً متوفراً رغم أنه ثبّته للتو.
// الحل: نقرأ رقم/اسم الإصدار الحقيقيين مباشرة من حزمة أندرويد المثبَّتة
// فعلياً (PackageManager عبر AndroidBridge، انظر MainActivity.java) - وهي
// دائماً صحيحة تلقائياً مع أي نسخة APK لأن أندرويد نفسه يضبطها، بدون أي
// حاجة لتذكّر تعديل رقم بملف منفصل. نبقي القيمتين اليدويتين كحد أدنى
// احتياطي فقط لحالة تشغيل هذا الكود كموقع PWA عادي بدون تطبيق أندرويد.
window.APP_VERSION_CODE = 1;
window.APP_VERSION_NAME = '1.0.0';

(function syncAppVersionFromNativeBridge() {
    try {
        if (window.AndroidBridge && typeof window.AndroidBridge.getAppVersionCode === 'function') {
            const nativeCode = parseInt(window.AndroidBridge.getAppVersionCode(), 10);
            if (nativeCode && nativeCode > 0) window.APP_VERSION_CODE = nativeCode;
        }
        if (window.AndroidBridge && typeof window.AndroidBridge.getAppVersionName === 'function') {
            const nativeName = window.AndroidBridge.getAppVersionName();
            if (nativeName) window.APP_VERSION_NAME = nativeName;
        }
    } catch (e) {
        console.warn('⚠️ تعذّر قراءة رقم الإصدار من التطبيق الأصلي، استخدام القيمة الاحتياطية:', e);
    }
})();

// ================================================================
//  إصلاح دقة "المتصلين الآن" بالشريط العلوي ولوحة المسؤول
// ================================================================
// المشكلة الأصلية: كان الاعتماد فقط على حقل online:true بمستند المستخدم،
// وهذا الحقل يُضبط false فقط عند حدث beforeunload. بتطبيق أندرويد داخل
// WebView (مو متصفح فيه تبويب)، حدث beforeunload لا يحدث بشكل موثوق أبداً
// عند: الضغط على زر الهوم (التطبيق ينتقل للخلفية)، سحب التطبيق من قائمة
// التطبيقات الأخيرة، أو انتهاء العملية من نظام أندرويد لتوفير الذاكرة. أي
// مستخدم يغلق التطبيق بأي طريقة غير "تسجيل خروج" صريح يبقى "online:true"
// للأبد بفايرستور، فيصير عدد "المتصلين" بالشريط العلوي أكبر من الحقيقة
// (أحياناً بمراحل) ولا ينزل أبداً. الحل: لا نثق بحقل online وحده، بل نتحقق
// أيضاً أن lastSeen حديث فعلاً (خلال ضعف فترة النبض 10 ثوانٍ + هامش أمان) —
// أي مستخدم توقف عن إرسال النبض (تعطل، خلفية، إغلاق قسري) يُعتبر غير متصل
// تلقائياً خلال ~35 ثانية كحد أقصى حتى لو حقل online بقي true بالخطأ.
window.PRESENCE_STALE_MS = 35000;
function isUserActuallyOnline(data) {
    if (!data || !data.online) return false;
    const lastSeen = data.lastSeen;
    if (!lastSeen || typeof lastSeen.toMillis !== 'function') {
        // لسه ما وصل timestamp حقيقي من السيرفر (لحظة الكتابة الأولى) —
        // نعتبره متصل مؤقتاً بدل إخفائه بالخطأ.
        return true;
    }
    return (Date.now() - lastSeen.toMillis()) < window.PRESENCE_STALE_MS;
}
window.settings = {
    muteAll: false,
    confirmDelete: true,
...
// ================================================================
//  هوية المستخدم الحقيقية (Firebase Auth UID)
// ================================================================
// نُرفق uid الحقيقي (موثّق من سيرفرات جوجل، غير قابل للتزوير من المتصفح)
// مع كل رسالة بالإضافة لاسم المستخدم النصي. هذا يسمح بتحقق حقيقي لاحقاً
// (بقاعدة أمان Firestore) من أن "صاحب الرسالة فقط يقدر يعدّلها/يحذفها" -
// بعكس username وحده اللي أي شخص يقدر يرسله كنص عادي بالطلب.
...
const fullEmojis = {
    ...
    '🤕': 'وجوه', '🤑': 'وجوه', '🤠': 'وجوه', '😈': 'وجوه', '👿': 'وجوه',
    '👹': 'وجوه', '👺': 'وجوه', '🤡': 'وجوه', '💩': 'وجوه', '👻': 'وجوه',
    '💀': 'وجوه', '☠️': 'وجوه', '👽': 'وجوه', '👾': 'وجوه', '🤖': 'وجوه',
    '🎃': 'وجوه', '🥲': 'وجوه', '🫠': 'وجوه', '🫡': 'وجوه', '🫢': 'وجوه',
    '🫣': 'وجوه', '🫤': 'وجوه', '🫨': 'وجوه', '🥸': 'وجوه', '😶‍🌫️': 'وجوه',
    '😵‍💫': 'وجوه',
    '👋': 'أشخاص', '🤚': 'أشخاص', '🖐️': 'أشخاص', '✋': 'أشخاص', '🖖': 'أشخاص',
    '👌': 'أشخاص', '🤌': 'أشخاص', '🤏': 'أشخاص', '✌️': 'أشخاص', '🤞': 'أشخاص',
    '🤟': 'أشخاص', '🤘': 'أشخاص', '🤙': 'أشخاص', '👈': 'أشخاص', '👉': 'أشخاص',
    '👆': 'أشخاص', '👇': 'أشخاص', '☝️': 'أشخاص', '👍': 'أشخاص', '👎': 'أشخاص',
    '✊': 'أشخاص', '👊': 'أشخاص', '🤛': 'أشخاص', '🤜': 'أشخاص', '👏': 'أشخاص',
    '🙌': 'أشخاص', '👐': 'أشخاص', '🤲': 'أشخاص', '🤝': 'أشخاص', '🙏': 'أشخاص',
    '✍️': 'أشخاص', '💅': 'أشخاص', '🤳': 'أشخاص', '💪': 'أشخاص', '🦾': 'أشخاص',
    '👶': 'أشخاص', '🧒': 'أشخاص', '👦': 'أشخاص', '👧': 'أشخاص', '🧑': 'أشخاص',
    '👨': 'أشخاص', '👩': 'أشخاص', '🧓': 'أشخاص', '👴': 'أشخاص', '👵': 'أشخاص',
    '🫱': 'أشخاص', '🫲': 'أشخاص', '🫳': 'أشخاص', '🫴': 'أشخاص', '🫰': 'أشخاص',
    '🫵': 'أشخاص', '🦵': 'أشخاص', '🦶': 'أشخاص', '👂': 'أشخاص', '🦻': 'أشخاص',
    '👃': 'أشخاص', '🧠': 'أشخاص', '🦷': 'أشخاص', '🦴': 'أشخاص', '👀': 'أشخاص',
    '👁️': 'أشخاص', '👅': 'أشخاص', '👄': 'أشخاص', '🫦': 'أشخاص', '👤': 'أشخاص',
    '👥': 'أشخاص', '🗣️': 'أشخاص', '👱': 'أشخاص', '👮': 'أشخاص', '🕵️': 'أشخاص',
    '💂': 'أشخاص', '🥷': 'أشخاص', '👷': 'أشخاص', '🤴': 'أشخاص', '👸': 'أشخاص',
    '👳': 'أشخاص', '👲': 'أشخاص', '🧕': 'أشخاص', '🤵': 'أشخاص', '👰': 'أشخاص',
    '🤰': 'أشخاص', '🤱': 'أشخاص', '👼': 'أشخاص', '🎅': 'أشخاص', '🤶': 'أشخاص',
    '🦸': 'أشخاص', '🦹': 'أشخاص', '🧙': 'أشخاص', '🧚': 'أشخاص', '🧛': 'أشخاص',
    ...
    '💇': 'أشخاص', '🚶': 'أشخاص', '🧍': 'أشخاص', '🧎': 'أشخاص', '🏃': 'أشخاص',
    '💃': 'أشخاص', '🕺': 'أشخاص', '👯': 'أشخاص', '🧖': 'أشخاص', '🏇': 'أشخاص',
    '⛷️': 'أشخاص', '🏂': 'أشخاص', '🏌️': 'أشخاص', '🏄': 'أشخاص', '🚣': 'أشخاص',
    '🏊': 'أشخاص', '🏋️': 'أشخاص', '🚴': 'أشخاص', '🚵': 'أشخاص', '🤸': 'أشخاص',
    '🧘': 'أشخاص', '🛀': 'أشخاص', '🛌': 'أشخاص', '👭': 'أشخاص', '👫': 'أشخاص',
    '👬': 'أشخاص', '💏': 'أشخاص', '💑': 'أشخاص', '👪': 'أشخاص',
    ...
    '🐩': 'حيوانات', '🐎': 'حيوانات', '🦌': 'حيوانات', '🦬': 'حيوانات', '🐂': 'حيوانات',
    '🐃': 'حيوانات', '🐏': 'حيوانات', '🦙': 'حيوانات', '🦏': 'حيوانات', '🐁': 'حيوانات',
    '🐀': 'حيوانات', '🐿️': 'حيوانات', '🦔': 'حيوانات', '🦢': 'حيوانات', '🦩': 'حيوانات',
    '🕊️': 'حيوانات', '🐇': 'حيوانات', '🦝': 'حيوانات', '🦨': 'حيوانات', '🦡': 'حيوانات',
    '🦫': 'حيوانات', '🦦': 'حيوانات', '🦥': 'حيوانات', '🐾': 'حيوانات', '🐉': 'حيوانات',
    '🐲': 'حيوانات', '🦕': 'حيوانات', '🦖': 'حيوانات', '🐡': 'حيوانات', '🦭': 'حيوانات',
    '🦞': 'حيوانات', '🦐': 'حيوانات', '🦪': 'حيوانات', '🐚': 'حيوانات',
    '🌸': 'طبيعة', '💐': 'طبيعة', '🌷': 'طبيعة', '🌹': 'طبيعة', '🌻': 'طبيعة',
    '🌼': 'طبيعة', '🌱': 'طبيعة', '🌲': 'طبيعة', '🌳': 'طبيعة', '🌴': 'طبيعة',
    '🍀': 'طبيعة', '🍁': 'طبيعة', '🍂': 'طبيعة', '🍃': 'طبيعة', '🌵': 'طبيعة',
    '🌾': 'طبيعة', '🌊': 'طبيعة', '🔥': 'طبيعة', '⭐': 'طبيعة', '🌟': 'طبيعة',
    '✨': 'طبيعة', '⚡': 'طبيعة', '☀️': 'طبيعة', '🌤️': 'طبيعة', '⛅': 'طبيعة',
    '🌧️': 'طبيعة', '⛈️': 'طبيعة', '🌈': 'طبيعة', '☁️': 'طبيعة', '❄️': 'طبيعة',
    '☃️': 'طبيعة', '🌙': 'طبيعة', '🌛': 'طبيعة', '🌎': 'طبيعة', '🌺': 'طبيعة',
    '🌿': 'طبيعة', '☘️': 'طبيعة', '🍄': 'طبيعة', '🌰': 'طبيعة', '🪨': 'طبيعة',
    '🪵': 'طبيعة', '🌍': 'طبيعة', '🌏': 'طبيعة', '🌑': 'طبيعة', '🌕': 'طبيعة',
    '🌗': 'طبيعة', '🌘': 'طبيعة', '🌚': 'طبيعة', '🌝': 'طبيعة', '🌞': 'طبيعة',
    '💫': 'طبيعة', '🌠': 'طبيعة', '🎆': 'طبيعة', '🎇': 'طبيعة', '🧨': 'طبيعة',
    '☂️': 'طبيعة', '☔': 'طبيعة', '⛱️': 'طبيعة', '🌡️': 'طبيعة', '🌪️': 'طبيعة',
    '🌫️': 'طبيعة', '🌬️': 'طبيعة', '🌀': 'طبيعة',
    ...
    '🎬': 'نشاطات', '🏉': 'نشاطات', '🎳': 'نشاطات', '🏒': 'نشاطات', '🏑': 'نشاطات',
    '🥍': 'نشاطات', '🏏': 'نشاطات', '🥊': 'نشاطات', '🥋': 'نشاطات', '🎽': 'نشاطات',
    '🛹': 'نشاطات', '🛼': 'نشاطات', '⛸️': 'نشاطات', '🥌': 'نشاطات', '🎿': 'نشاطات',
    '🏹': 'نشاطات', '🎣': 'نشاطات', '🤿': 'نشاطات', '🥏': 'نشاطات', '🪀': 'نشاطات',
    '🪁': 'نشاطات', '🎰': 'نشاطات', '🃏': 'نشاطات', '🀄': 'نشاطات', '🎴': 'نشاطات',
    '🧩': 'نشاطات', '🪄': 'نشاطات', '🎭': 'نشاطات', '🩰': 'نشاطات', '🎼': 'نشاطات',
    '🎵': 'نشاطات', '🎶': 'نشاطات', '🥁': 'نشاطات', '🪘': 'نشاطات', '🎺': 'نشاطات',
    '🎻': 'نشاطات', '🪕': 'نشاطات', '📯': 'نشاطات',
    '🎁': 'أشياء', '📷': 'أشياء', '💻': 'أشياء', '📱': 'أشياء', '⌚': 'أشياء',
    '🎥': 'أشياء', '📺': 'أشياء', '🔋': 'أشياء', '💡': 'أشياء', '🔦': 'أشياء',
    '💰': 'أشياء', '💵': 'أشياء', '💳': 'أشياء', '📚': 'أشياء', '✏️': 'أشياء',
    '📝': 'أشياء', '📌': 'أشياء', '🔑': 'أشياء', '🔒': 'أشياء', '⏰': 'أشياء',
    '🖥️': 'أشياء', '🖨️': 'أشياء', '⌨️': 'أشياء', '🖱️': 'أشياء', '💽': 'أشياء',
    '💾': 'أشياء', '💿': 'أشياء', '📀': 'أشياء', '📼': 'أشياء', '☎️': 'أشياء',
    '📞': 'أشياء', '📟': 'أشياء', '📠': 'أشياء', '📡': 'أشياء', '🔌': 'أشياء',
    '🧯': 'أشياء', '🛢️': 'أشياء', '🪔': 'أشياء', '🧵': 'أشياء', '🪡': 'أشياء',
    '🧶': 'أشياء', '🪢': 'أشياء', '👓': 'أشياء', '🕶️': 'أشياء', '🥽': 'أشياء',
    '🥼': 'أشياء', '🦺': 'أشياء', '👔': 'أشياء', '👕': 'أشياء', '👖': 'أشياء',
    '🧣': 'أشياء', '🧤': 'أشياء', '🧥': 'أشياء', '🧦': 'أشياء', '👗': 'أشياء',
    '👘': 'أشياء', '🥻': 'أشياء', '🩱': 'أشياء', '🩲': 'أشياء', '🩳': 'أشياء',
    '👙': 'أشياء', '👚': 'أشياء', '👛': 'أشياء', '👜': 'أشياء', '👝': 'أشياء',
    '🛍️': 'أشياء', '🎒': 'أشياء', '👞': 'أشياء', '👟': 'أشياء', '🥾': 'أشياء',
    '🥿': 'أشياء', '👠': 'أشياء', '👡': 'أشياء', '👢': 'أشياء', '👑': 'أشياء',
    '💍': 'أشياء', '💄': 'أشياء', '💼': 'أشياء', '📁': 'أشياء', '📂': 'أشياء',
    '🗂️': 'أشياء', '📅': 'أشياء', '📆': 'أشياء', '🗒️': 'أشياء', '🗓️': 'أشياء',
    '📇': 'أشياء', '📈': 'أشياء', '📉': 'أشياء', '📊': 'أشياء', '📋': 'أشياء',
    '📍': 'أشياء', '📎': 'أشياء', '🖇️': 'أشياء', '📏': 'أشياء', '📐': 'أشياء',
    '✂️': 'أشياء', '🗃️': 'أشياء', '🗄️': 'أشياء', '🗑️': 'أشياء', '🔨': 'أشياء',
    '🪓': 'أشياء', '⛏️': 'أشياء', '⚒️': 'أشياء', '🛠️': 'أشياء', '🗡️': 'أشياء',
    '⚔️': 'أشياء', '🔫': 'أشياء', '🛡️': 'أشياء', '🔧': 'أشياء', '🔩': 'أشياء',
    '⚙️': 'أشياء', '🗜️': 'أشياء', '⚖️': 'أشياء', '🦯': 'أشياء', '🔗': 'أشياء',
    '⛓️': 'أشياء', '🧰': 'أشياء', '🧲': 'أشياء', '⚗️': 'أشياء', '🧪': 'أشياء',
    '🧫': 'أشياء', '🧬': 'أشياء', '🔬': 'أشياء', '🔭': 'أشياء', '💉': 'أشياء',
    '🩸': 'أشياء', '💊': 'أشياء', '🩹': 'أشياء', '🩺': 'أشياء', '🚪': 'أشياء',
    '🛗': 'أشياء', '🪞': 'أشياء', '🪟': 'أشياء', '🛏️': 'أشياء', '🛋️': 'أشياء',
    '🪑': 'أشياء', '🚽': 'أشياء', '🚿': 'أشياء', '🛁': 'أشياء', '🪤': 'أشياء',
    '🪒': 'أشياء', '🧴': 'أشياء', '🧷': 'أشياء', '🧹': 'أشياء', '🧺': 'أشياء',
    '🧻': 'أشياء', '🪣': 'أشياء', '🧼': 'أشياء', '🫧': 'أشياء', '🪥': 'أشياء',
    '🧽': 'أشياء', '🪠': 'أشياء',
    ...
    '🚠': 'سفر', '🚃': 'سفر', '🚋': 'سفر', '🚞': 'سفر', '🚝': 'سفر',
    '🚄': 'سفر', '🚅': 'سفر', '🚈': 'سفر', '🚂': 'سفر', '🛳️': 'سفر',
    '🛥️': 'سفر', '⛴️': 'سفر', '🛶': 'سفر', '🚊': 'سفر', '🚉': 'سفر',
    '🛫': 'سفر', '🛬': 'سفر', '🛰️': 'سفر', '🚦': 'سفر', '🚧': 'سفر',
    '⚓': 'سفر', '⛽': 'سفر', '🗽': 'سفر', '🗼': 'سفر', '🏰': 'سفر',
    '🏯': 'سفر', '🎡': 'سفر', '🎢': 'سفر', '🎠': 'سفر', '⛲': 'سفر',
    '⛺': 'سفر', '🌋': 'سفر', '🗻': 'سفر',
    ...
    '🕐': 'رموز', '💟': 'رموز', '✔️': 'رموز', '☑️': 'رموز', '➕': 'رموز',
    '➖': 'رموز', '➗': 'رموز', '✖️': 'رموز', '♾️': 'رموز', '💱': 'رموز',
    '💲': 'رموز', '⚕️': 'رموز', '⚠️': 'رموز', '🚸': 'رموز', '🔱': 'رموز',
    '📛': 'رموز', '🔰': 'رموز', '🔴': 'رموز', '🟠': 'رموز', '🟡': 'رموز',
    '🟢': 'رموز', '🔵': 'رموز', '🟣': 'رموز', '🟤': 'رموز', '⚫': 'رموز',
    '⚪': 'رموز', '🟥': 'رموز', '🟧': 'رموز', '🟨': 'رموز', '🟩': 'رموز',
    '🟦': 'رموز', '🟪': 'رموز', '🟫': 'رموز', '⬛': 'رموز', '⬜': 'رموز',
    '◼️': 'رموز', '◻️': 'رموز', '◾': 'رموز', '◽': 'رموز', '▪️': 'رموز',
    '▫️': 'رموز', '🔶': 'رموز', '🔷': 'رموز', '🔸': 'رموز', '🔹': 'رموز',
    '🔺': 'رموز', '🔻': 'رموز', '💠': 'رموز', '🔘': 'رموز', '🔲': 'رموز',
    '🔳': 'رموز', '🈴': 'رموز', '🈵': 'رموز', '🈹': 'رموز', '🈲': 'رموز',
    '🈶': 'رموز', '🈚': 'رموز', '🈸': 'رموز', '🈺': 'رموز', '🈷️': 'رموز',
    '✴️': 'رموز', '🆚': 'رموز', '💮': 'رموز', '🉐': 'رموز', '🔔': 'رموز',
    '🔕': 'رموز', '📢': 'رموز', '📣': 'رموز', '🔇': 'رموز', '🔈': 'رموز',
    '🔉': 'رموز', '🔊': 'رموز', '💬': 'رموز', '🗨️': 'رموز', '🗯️': 'رموز',
    '💭': 'رموز', '🕳️': 'رموز', '♠️': 'رموز', '♣️': 'رموز', '♦️': 'رموز',
...
// أيقونة تمثيلية لكل تصنيف بشريط القفز السريع فوق لوحة الإيموجي
...
// سجل آخر الإيموجي المستخدَمة محلياً (حتى 24 عنصر، الأحدث أولاً) - يُبنى
// منه تصنيف "الأخيرة" أعلى اللوحة فيصل المستخدم لرموزه المفضّلة بضغطة
// واحدة بدل التمرير لتصنيفها الأصلي في كل مرة.
function getRecentEmojis() {
    try {
        return JSON.parse(localStorage.getItem('tomodachi_recentEmojis') || '[]');
    } catch (e) { return []; }
}
function addRecentEmoji(emoji) {
    try {
        ...
        localStorage.setItem('tomodachi_recentEmojis', JSON.stringify(recent));
    } catch (e) {}
}

function initFullEmojiPanel() {
    const panel = document.getElementById('emojiPanel');
    const tabsBar = document.getElementById('emojiCategoryTabs');
    if (!panel) return;
    panel.innerHTML = '';
    const categories = getEmojiCategories();
    const recent = getRecentEmojis();
    if (recent.length) {
        categories['الأخيرة'] = recent;
    }
    // إعادة ترتيب: "الأخيرة" أولاً لو وُجدت، ثم باقي التصنيفات بترتيبها الأصلي
    const orderedEntries = Object.entries(categories).sort((a, b) => {
        if (a[0] === 'الأخيرة') return -1;
        if (b[0] === 'الأخيرة') return 1;
        return 0;
    });

    for (const [category, emojis] of orderedEntries) {
        const catLabel = document.createElement('div');
        catLabel.className = 'emoji-category';
        catLabel.id = 'emoji-cat-' + encodeURIComponent(category);
        catLabel.dataset.categoryName = category;
        ...
        emojis.forEach(emoji => {
            ...
            span.dataset.categoryName = category;
            ...
            span.onclick = function() {
                ...
                const searchBox = document.getElementById('emojiSearchBox');
    ...
    // شريط القفز السريع بين التصنيفات (مثل إنستغرام/واتساب)
    if (tabsBar) {
        tabsBar.innerHTML = '';
        orderedEntries.forEach(([category], idx) => {
            const tab = document.createElement('span');
            tab.className = 'emoji-cat-tab' + (idx === 0 ? ' active' : '');
            tab.textContent = emojiCategoryIcons[category] || '🔸';
            tab.title = category;
            tab.onclick = function() {
                const target = document.getElementById('emoji-cat-' + encodeURIComponent(category));
                if (target) target.scrollIntoView({ block: 'start', behavior: 'smooth' });
                tabsBar.querySelectorAll('.emoji-cat-tab').forEach(t => t.classList.remove('active'));
...
// فلترة تصنيفات لوحة الإيموجي حسب اسم التصنيف المطابق لنص البحث - تصنيف
// كامل يُخفى أو يُظهر معاً (تسميته + كل رموزه) دفعة واحدة.
function filterEmojiPanel(query) {
    const panel = document.getElementById('emojiPanel');
    if (!panel) return;
    const q = (query || '').trim();
    const nodes = panel.querySelectorAll('[data-category-name]');
    if (!q) {
        nodes.forEach(el => { el.style.display = ''; });
        return;
    }
    nodes.forEach(el => {
        const matches = el.dataset.categoryName.includes(q);
    ...
    try {
        ...
        if (doc.exists) {
            ...
            if (data.stickers && data.stickers.length > 0) {
                ...
                if (typeof window.mergeExtraStickerPacks === 'function') window.mergeExtraStickerPacks();
...
// المفضّلة: قائمة معرّفات ستيكرات محفوظة محلياً - تبويب سريع للوصول
// لأكثر الستيكرات استخداماً بدل البحث عنها بين الحزم في كل مرة.
function getFavoriteStickerIds() {
    try {
        return JSON.parse(localStorage.getItem('tomodachi_favStickers') || '[]');
    } catch (e) { return []; }
}
function toggleFavoriteSticker(id) {
    let favs = getFavoriteStickerIds();
    if (favs.includes(id)) {
        favs = favs.filter(f => f !== id);
    } else {
        favs.push(id);
    }
    localStorage.setItem('tomodachi_favStickers', JSON.stringify(favs));
...
function renderStickers(tabId) {
    ...
    const favIds = getFavoriteStickerIds();
    if (tabId === 'fav_stickers') {
        stickers = window.allStickers.filter(s => favIds.includes(s.id));
    } else if (tabId !== 'my_stickers' && tabId !== 'all_stickers') {
    ...
    if (!stickers || stickers.length === 0) {
        const isFavTab = tabId === 'fav_stickers';
        grid.innerHTML = `<div class="chat-panel-empty-state"><span class="empty-icon">${isFavTab ? '⭐' : '🎨'}</span><span>${isFavTab ? 'لا توجد ستيكرات مفضّلة بعد — اضغط ⭐ على أي ستيكر لإضافته' : 'لا توجد ستيكرات'}</span></div>`;
        return;
    }
    grid.innerHTML = '';
    stickers.forEach(sticker => {
        ...
        const isFav = favIds.includes(sticker.id);
        const starHtml = `<span class="fav-star${isFav ? ' active' : ''}" data-sticker-id="${escapeJsAttr(sticker.id)}">⭐</span>`;
        if (sticker.type === 'image' && sticker.data) {
            span.innerHTML = `<img src="${escapeHtml(sticker.data)}" alt="${escapeHtml(sticker.name || 'ستيكر')}" title="بواسطة: ${escapeHtml(sticker.uploadedBy || 'غير معروف')}" />${starHtml}`;
        } else {
            span.innerHTML = `<span>${escapeHtml(sticker.emoji || '😊')}</span>${starHtml}`;
            span.title = sticker.uploadedBy ? `بواسطة: ${sticker.uploadedBy}` : '';
        }
        span.onclick = function(e) {
            if (e.target.classList.contains('fav-star')) {
                e.stopPropagation();
                const nowFav = toggleFavoriteSticker(sticker.id);
                e.target.classList.toggle('active', nowFav);
                if (window.currentStickerTab === 'fav_stickers' && !nowFav) {
                    renderStickers('fav_stickers');
                }
                return;
            }
            if (sticker.type === 'image' && sticker.data) {
...
function renderStickerTabs() {
    ...
    const favTab = document.createElement('span');
    favTab.className = 'tab' + (window.currentStickerTab === 'fav_stickers' ? ' active' : '');
    favTab.textContent = '🌟 المفضّلة';
    favTab.onclick = function() {
        window.currentStickerTab = 'fav_stickers';
        renderStickerTabs();
        renderStickers(window.currentStickerTab);
    };
...
async function checkBadWords(text, username) {
    ...
    if (foundWords.length > 0) {
        ...
        // إصلاح: banExpires الآن رقم ميلي ثانية حقيقي حسبه هذا الجهاز (Date.now())
        // بدل جمع رقم مع كائن FieldValue.serverTimestamp() الذي لا ينتج وقتاً
        // صالحاً إطلاقاً. أي جهاز (بما فيه جهاز المستخدم نفسه عند فتح التطبيق
        // من جديد، أو جهاز أي مسؤول) يقدر الآن يتحقق فعلياً هل انتهت مدة
        // الحظر أم لا عبر isBanStillActive()، بدل الاعتماد فقط على مؤقّت
        // setTimeout بمتصفح المستخدم المحظور نفسه (الذي على الأغلب أغلق
        // التطبيق للتو بسبب الحظر، فلا يُنفَّذ المؤقّت أبداً ويبقى محظوراً للأبد).
        const banExpiresMs = Date.now() + banDuration * 1000;
        await db.collection('users').doc(username).update({
            banned: true,
            banReason: `استخدام كلمات ممنوعة (${foundWords.join(', ')})`,
            banExpires: banExpiresMs,
            banCount: count
        });
        ...
        setTimeout(async () => {
            ...
            } catch (e) { /* تجاهل - سيُتحقق من isBanStillActive عند أي محاولة دخول لاحقة بكل الأحوال */ }
        }, banDuration * 1000);
        return true;
    }
    return false;
...
async function sendMessage() {
    ...
    // نبضة تفاعلية فورية على زر الإرسال بلحظة الضغط الفعلي - لا تنتظر أي
    // شيء (لا الشبكة ولا حتى بناء الرسالة بالأسفل)، فتصل ردة الفعل البصرية
    // للمستخدم بأسرع وقت ممكن (راجع .send-btn.just-sent بـ index.html).
    const sendBtnEl = document.getElementById('sendBtn');
    if (sendBtnEl) {
        sendBtnEl.classList.remove('just-sent');
        void sendBtnEl.offsetWidth; // إعادة تشغيل الأنيميشن حتى مع ضغطات متتالية سريعة
        sendBtnEl.classList.add('just-sent');
    }
    ...
    if (window.replyingTo) {
        ...
        // إصلاح: اسم صاحب الرسالة الأصلية لم يكن يُحفظ إطلاقاً، فكانت معاينة
        // الرد بأي رسالة تالية تعرض المقتطف النصي فقط بدون قائل الجملة.
        msgData.replyToUser = window.replyingToUser || '';
    }
    ...
    document.getElementById('emojiPanel').style.display = 'none';
    document.getElementById('stickerPanel').style.display = 'none';
    const emojiCatTabsEl = document.getElementById('emojiCategoryTabs');
    if (emojiCatTabsEl) emojiCatTabsEl.style.display = 'none';
    const emojiSearchBoxEl = document.getElementById('emojiSearchBox');
    if (emojiSearchBoxEl) emojiSearchBoxEl.style.display = 'none';

    window.messageCount = window.allMessages.length;
    adjustTotalMessageCount(1);
...
// ================================================================
//  العدد الحقيقي الكلي للرسائل (للشريط العلوي)
// ================================================================
// نجلبه مرة واحدة عند الدخول عبر استعلام تجميعي (count) خفيف من السيرفر
// (لا يقرأ كل المستندات، فقط رقم العدّ) بدل الاعتماد على طول المصفوفة
// المحمَّلة محلياً، ثم نعدّله تدريجياً محلياً (+1/-1) مع كل رسالة تُرسل أو
// تُحذف بدل إعادة الاستعلام من السيرفر في كل مرة.
window.totalMessageCount = null;

async function initTotalMessageCount() {
    try {
        const snap = await db.collection('messages').where('deleted', '==', false).count().get();
        window.totalMessageCount = snap.data().count;
    } catch (error) {
        console.warn('⚠️ فشل جلب العدد الكلي للرسائل، استخدام العدد المحمَّل محلياً:', error);
        window.totalMessageCount = window.allMessages.length;
    }
    scheduleUpdateStats();
}

function adjustTotalMessageCount(delta) {
    if (typeof window.totalMessageCount !== 'number') return;
    window.totalMessageCount = Math.max(0, window.totalMessageCount + delta);
    scheduleUpdateStats();
}
...
async function loadMessages() {
    try {
        ...
        // إصلاح: كان هناك جلب منفصل لمرة واحدة هنا (آخر 30 رسالة عبر .get())
        // بالإضافة لمستمع حي منفصل (listenMessages، آخر 50 رسالة) يُشغَّل بعد
        // 500ms من enterChat. هذا كان يعني طلب شبكة مكرر تقريباً لنفس البيانات،
        // واعتماد ضمني وهش على أن نصف ثانية تكفي لإنهاء الجلب الأول قبل بدء
        // المستمع (وإلا تُعامَل الرسائل القديمة كرسائل جديدة عند أول نبضة من
        // المستمع - راجع isInitialSnapshot في listenMessages). الآن المستمع
        // الحي هو المصدر الوحيد للرسائل من الشبكة، ويُستدعى مباشرة من enterChat
        // بدون تأخير. هنا نكتفي بعرض فوري من الكاش المحلي (بدون شبكة).
...
function listenMessages() {
    ...
    // إصلاح جوهري لتحميل/عرض الرسائل: كان يوجد مصدران منفصلان للرسائل عند
    // الدخول للمحادثة - جلب لمرة واحدة (loadMessagesFromFirebase، آخر 30
    // رسالة) ثم بعد 500ms (رقم اعتباطي) هذا المستمع الحي هنا (آخر 50 رسالة)
    // يبدأ من جديد. مشكلتان حقيقيتان بهذا: (1) طلب بيانات مكرر تقريباً لنفس
    // الرسائل، و(2) الاعتماد الضمني على أن الـ 500ms كافية لتنتهي الدفعة
    // الأولى قبل أن يبدأ المستمع - لو تأخرت الشبكة، أول نبضة من المستمع
    // كانت تُعامل كل الرسائل القديمة (حتى 50) على أنها "رسائل جديدة" فعلياً
    // فتُطلق إشعارات وشارة "غير مقروء" لرسائل قديمة أصلاً عند أول فتح للتطبيق.
    // الحل: هذا المستمع هو المصدر الوحيد للرسائل الحية، ونميّز نبضته الأولى
    // صراحة بعلم isInitialSnapshot بدل الاعتماد على توقيت عشوائي.
    ...
    window.messagesListener = db.collection('messages')
        ...
        .onSnapshot((snapshot) => {
            try {
                if (snapshot.empty) { isInitialSnapshot = false; return; }

                const isFirstLoad = isInitialSnapshot;
                ...
                changes.forEach(change => {
                    ...
                    if (change.type === 'removed' || data.deleted === true) {
                        if (window.messageCache.has(doc.id)) {
                            ...
                            if (typeof adjustTotalMessageCount === 'function') adjustTotalMessageCount(-1);
                        }
                        return;
                    }
                ...
                // مرساة الصفحات لزر "تحميل المزيد" (كانت تُضبط فقط من الجلب
                // اليدوي المنفصل الذي أزلناه؛ الآن تُضبط من دفعة المستمع الأولى)
                if (isFirstLoad && snapshot.docs.length > 0) {
                    window.lastVisibleDoc = snapshot.docs[snapshot.docs.length - 1];
                    if (snapshot.docs.length >= 50) addLoadMoreButton();
                }
                ...
                if (newMessages.length > 0) {
                    ...
                    if (isFirstLoad) {
                        // تحميل أولي فقط: مجرد عرض ونزول مباشر للأسفل، بدون
                        // معاملتها كرسائل جديدة (لا إشعارات، لا شارة غير مقروء)
                        setTimeout(() => {
                            container.scrollTop = container.scrollHeight;
                        }, 50);
                    } else {
                        // رسائل جديدة فعلياً (من مستخدمين آخرين على الأغلب - رسائلنا
                        // نحن تُحتسب فوراً عند الإرسال المتفائل بأعلى)
                        adjustTotalMessageCount(newMessages.length);
...
function renderMessages(messages) {
    ...
    // تتبّع معرّفات الرسائل المعروضة فعلياً بالتصيير السابق - يحدّد أي
    // رسائل بهذا التصيير جديدة فعلاً (تستحق أنيميشن دخول) وأيها كانت
    // ظاهرة أصلاً ويجب أن تبقى ثابتة رغم إعادة بناء innerHTML بالكامل.
    // أول تصيير للمحادثة (فتح التطبيق/تبديل حساب) لا يُشار فيه لأي رسالة
    // كـ"جديدة" عمداً - محادثة طويلة لا يجب أن تتحرك كل رسائلها التاريخية
    // دفعة واحدة عند أول فتح، فقط ما يصل لاحقاً وهي مفتوحة.
    const isFirstRender = !window.__renderedMsgIds;
    const previouslyRendered = window.__renderedMsgIds || new Set();
    ...
    sorted.forEach(data => {
        ...
        if (data.replyTo && data.replyText) {
            // إصلاح: معاينة الرد كانت لا تُظهر اسم صاحب الرسالة الأصلية أبداً
            // (فقط مقتطف من نصها)، ولم يكن هناك أي طريقة للانتقال إليها عند
            // الضغط عليها. الآن نعرض اسم صاحبها (data.replyToUser، مع نص
            // احتياطي للرسائل القديمة قبل إضافة هذا الحقل) ونجعل المعاينة
            // قابلة للضغط للانتقال والتمييز اللحظي للرسالة الأصلية عبر
            // scrollToMessage - ما دامت لا تزال محمَّلة بالذاكرة.
            const replyTextDecompressed = escapeHtml(decompressText(data.replyText));
            const replyToIdAttr = escapeJsAttr(data.replyTo);
            const replyToUserSafe = escapeHtml(data.replyToUser || 'رسالة محذوفة/قديمة');
            replyHtml = `
                <div class="reply-preview" onclick="scrollToMessage('${replyToIdAttr}')" title="الانتقال للرسالة الأصلية">
                    <span class="reply-name">↩️ ${replyToUserSafe}</span>
                    <span class="reply-snippet">${replyTextDecompressed.substring(0, 35)}${replyTextDecompressed.length > 35 ? '...' : ''}</span>
                </div>
            `;
        }
        ...
        const isNewMessage = !isFirstRender && !previouslyRendered.has(data.id);
        const enteringClass = isNewMessage ? ' msg-entering' : '';
        // تدرّج بسيط بالتأخير عند وصول أكثر من رسالة جديدة دفعة واحدة
        // (مثلاً بعد عودة الاتصال بالإنترنت) بدل ظهورها كلها بنفس اللحظة
        // بالضبط - يُحدَّد الحد الأقصى بـ 6 رسائل حتى لا تتراكم رسائل
        // كثيرة بتأخير طويل يشعر المستخدم ببطء غير مبرر.
        const staggerDelay = isNewMessage ? Math.min(newMessageIndex, 6) * 45 : 0;
        const enteringStyle = isNewMessage && staggerDelay > 0 ? ` style="animation-delay:${staggerDelay}ms"` : '';
        if (isNewMessage) newMessageIndex++;

        html += `
            <div class="msg-wrapper${statusClass}${enteringClass}"${enteringStyle}
                 data-id="${idAttr}" 
                 data-text="${decompressedText}"
                 data-sender="${senderAttr}"
    ...
    // نحدّث سجلّ المعرّفات المعروضة بعد كل تصيير - يستخدمه التصيير التالي
    // ليعرف بالضبط أي رسائل جديدة فعلاً وتستحق أنيميشن دخول (راجع التعليق
    // أعلى تعريف isFirstRender بهذه الدالة).
    window.__renderedMsgIds = currentIds;
...
function handleSwipeEnd(e) {
    ...
    if (msg) {
        ...
        if (deltaX > 50) {
            ...
            const messageSender = window.swipeTarget.dataset.sender || '';
            if (messageId) { startReply(messageId, messageText, messageSender); }
        }
        msg.style.transform = '';
        msg.style.opacity = '';
    }
...
function startReply(messageId, messageText, senderName) {
    window.replyingTo = messageId;
    window.replyingToText = messageText;
    window.replyingToUser = senderName || window.currentUser;
    document.getElementById('replyBar').style.display = 'flex';
    const messageTextSafe = escapeHtml(messageText);
    const senderSafe = escapeHtml(window.replyingToUser);
    document.getElementById('replyText').innerHTML =
        `<span class="reply-name">الرد على ${senderSafe}:</span> ${messageTextSafe.substring(0, 45)}${messageTextSafe.length > 45 ? '...' : ''}`;
    document.getElementById('messageInput').focus();
}

// إصلاح: الانتقال للرسالة الأصلية عند الضغط على معاينة "الرد" - كانت
// المعاينة سابقاً غير قابلة للضغط إطلاقاً.
window.scrollToMessage = function(messageId) {
    if (!messageId) return;
    const el = document.querySelector(`.msg-wrapper[data-id="${CSS.escape(messageId)}"]`);
    if (!el) {
        showNotification('❌ الرسالة الأصلية غير محمَّلة حالياً (قد تحتاج "تحميل المزيد")');
        return;
    }
    el.scrollIntoView({ behavior: 'smooth', block: 'center' });
    el.classList.remove('highlight-flash');
    void el.offsetWidth; // إعادة تشغيل الأنيميشن لو ضُغط أكثر من مرة متتالية
...
window.editMessage = function(messageId) {
    ...
    db.collection('messages').doc(messageId).get().then(doc => {
        if (doc.exists) {
            const data = doc.data();
            // تحقق ملكية مزدوج: uid الحقيقي (Firebase Auth) لو متوفر بالرسالة،
            // وإلا الاسم النصي كخيار احتياطي للرسائل القديمة قبل إضافة uid.
            const myUid = getCurrentUid();
            const owns = data.uid ? (myUid && data.uid === myUid) : (data.username === window.currentUser);
            // إصلاح: تعديل النص صار مقصوراً على صاحب الرسالة فقط (بدون استثناء
            // للمسؤول) - راجع تعليق شرط isSent بأعلى showReactionBar.
            if (owns) {
...
window.deleteMessage = async function(id) {
    ...
    try {
        // إصلاح ثغرة أمان حقيقية: هذه الدالة كانت تحذف أي رسالة بأي id بدون
        // أي تحقق من الملكية إطلاقاً (بعكس editMessage اللي كان يتحقق فعلاً) -
        // أي مستخدم يقدر يستدعيها بمعرّف رسالة شخص آخر ويحذفها. الآن نتحقق
        // أولاً من doc.data() نفس تحقق التعديل بالضبط قبل أي عملية حذف.
        if (!window.isAdmin) {
            const doc = await db.collection('messages').doc(id).get();
            if (!doc.exists) { showNotification('❌ الرسالة غير موجودة'); return; }
            const data = doc.data();
            const myUid = getCurrentUid();
            const owns = data.uid ? (myUid && data.uid === myUid) : (data.username === window.currentUser);
            if (!owns) { showNotification('❌ لا يمكنك حذف رسالة أخرى'); return; }
        }
...
async function showProfile(username) {
    try {
        ...
        const status = isUserActuallyOnline(data) ? '🟢 متصل الآن' : '⚫ غير متصل';
...
function openAvatarModal() {
    ...
    if (isValidImageUrl(window.myAvatar)) {
        preview.innerHTML = `<img src="${escapeHtml(window.myAvatar)}" alt="avatar" /><div class="upload-overlay">📷</div>`;
    } else {
        preview.innerHTML = `${escapeHtml(window.myAvatar)}<div class="upload-overlay">📷</div>`;
    }
    updateProfileFieldCounters();
...
// عدّاد أحرف حيّ لحقلي الاسم والنبذة بمودال تعديل البروفايل - يقترب لونه
// من التحذير عند اقتراب الحد الأقصى ومن الخطر عند بلوغه، بدل انتظار رسالة
// خطأ عند الحفظ لمعرفة أن النص تجاوز الطول المسموح.
function updateProfileFieldCounters() {
    const nameInput = document.getElementById('editNameInput');
    const bioInput = document.getElementById('editBioInput');
    const nameCounter = document.getElementById('editNameCounter');
    const bioCounter = document.getElementById('editBioCounter');
    function apply(input, counter) {
        if (!input || !counter) return;
        const max = parseInt(input.getAttribute('maxlength'), 10) || 0;
        const len = input.value.length;
        counter.textContent = `${len}/${max}`;
        counter.classList.toggle('limit-near', max > 0 && len >= max * 0.85 && len < max);
        counter.classList.toggle('limit-reached', max > 0 && len >= max);
    }
    apply(nameInput, nameCounter);
    apply(bioInput, bioCounter);
}
if (document.getElementById('editNameInput')) {
    document.getElementById('editNameInput').addEventListener('input', updateProfileFieldCounters);
}
if (document.getElementById('editBioInput')) {
    document.getElementById('editBioInput').addEventListener('input', updateProfileFieldCounters);
}
...
function listenUsers() {
    ...
    window.usersListener = db.collection('users').onSnapshot((snapshot) => {
        ...
        snapshot.forEach(doc => {
            ...
            const reallyOnline = isUserActuallyOnline(data);
            if (reallyOnline) online++;
            if (data.blocked) blocked++;
            if (data.isAdmin) adminCount++;
            if (window.isAdmin && doc.id !== window.currentUser) {
                const status = reallyOnline ? '🟢' : '⚫';
                const itemFilterClasses = [
                    reallyOnline ? 'is-online' : '',
                    data.isAdmin ? 'is-admin' : '',
                ...
                const bannedBadge = (data.banned && isBanStillActive(data)) ? '<span class="badge banned">⏱️ حظر مؤقت</span>' : '';
                // إصلاح تصميم: "حظر" (blocked - يدوي ودائم) و"محظور" (banned - تلقائي
                // ومؤقت بسبب كلمات ممنوعة) كانا يظهران بنفس التسمية والشارة تقريباً
                // بلوحة المسؤول رغم اختلافهما جذرياً بالسبب والمدة - يسبب لبساً حقيقياً
                // للمسؤول عند اتخاذ القرار. الآن كل حالة بتسمية وأيقونة مختلفة بوضوح.
                const warnBadge = (data.warnCount > 0) ? `<span class="badge warned">⚠️ ${data.warnCount}</span>` : '';
                const uidAttr = escapeJsAttr(doc.id);
                const uidSafe = escapeHtml(doc.id);
                const avatarSafe = escapeHtml(data.avatar);
                adminListHtml += `
                    <div class="user-item ${itemFilterClasses}">
                        <div class="info">
                            <div class="avatar ${reallyOnline ? 'status-online' : 'status-offline'}">${isValidImageUrl(data.avatar) ? `<img src="${avatarSafe}" />` : (avatarSafe || '👤')}</div>
                            <span class="name">${uidSafe}</span>
                            ${adminBadge}${blockedBadge}${bannedBadge}${warnBadge}
                            <span class="status">${status}</span>
                        </div>
                        <div class="actions-group">
                            ${data.blocked ? 
                                `<button class="btn-unblock" onclick="unblockUser('${uidAttr}')" title="فك الحظر الدائم">🔓 فك الحظر</button>` :
                                `<button class="btn-block" onclick="blockUser('${uidAttr}')" title="حظر دائم (يمنع الدخول نهائياً حتى يُفك يدوياً)">⛔ حظر دائم</button>`
                            }
                            ${(data.banned && isBanStillActive(data)) ? 
                                `<button class="btn-unblock" onclick="unbanUser('${uidAttr}')" title="إلغاء الحظر المؤقت التلقائي">🔓 إلغاء الحظر المؤقت</button>` : ''
                            }
                            <button class="btn-delete" onclick="deleteUser('${uidAttr}')" title="حذف الحساب نهائياً">🗑️</button>
                            ${!data.isAdmin && doc.id !== 'slx23m' ? 
                                `<button class="btn-admin" onclick="makeAdmin('${uidAttr}')" title="ترقية إلى مسؤول">⭐ ترقية</button>` : ''}
                            <button class="btn-warn" onclick="warnUser('${uidAttr}')" title="إرسال تحذير (حظر تلقائي بعد 3 تحذيرات)">⚠️ تحذير${data.warnCount ? ` (${data.warnCount}/3)` : ''}</button>
                        </div>
                    </div>
                `;
            }
        ...
        document.getElementById('statMessages').textContent = (typeof window.totalMessageCount === 'number') ? window.totalMessageCount : window.messageCount;
        ...
        if (window.isAdmin) {
            ...
            document.getElementById('adminUserList').innerHTML = adminListHtml ||
                '<div class="admin-empty-state"><span class="empty-icon">👥</span><span>لا يوجد مستخدمين</span></div>';

            const chipAll = document.getElementById('chipCountAll');
            const chipOnline = document.getElementById('chipCountOnline');
            const chipAdmin = document.getElementById('chipCountAdmin');
            const chipBlocked = document.getElementById('chipCountBlocked');
            if (chipAll) chipAll.textContent = Object.keys(window.allUsers).length;
            if (chipOnline) chipOnline.textContent = online;
            if (chipAdmin) chipAdmin.textContent = adminCount;
            if (chipBlocked) chipBlocked.textContent = blocked;

            // تعيد تطبيق فلتر البحث + شريحة الحالة الحالية (إن وجدا) بعد كل
            // تحديث حي للقائمة، وإلا تُفقَد الفلترة مع أول تحديث اتصال قادم
            // من فايرستور (كانت المشكلة الأصلية تخص البحث فقط، والآن تشمل
            // شرائح الفلترة الجديدة أيضاً).
            if (typeof window.applyAdminUserFilters === 'function') {
...
function updateStats() {
    // إصلاح: كان العدد المعروض بالشريط العلوي هو فقط طول مصفوفة الرسائل
    // المحمَّلة محلياً (window.messageCount) — يبدأ بأي رقم حسب آخر دفعة
    // محمَّلة، ويتغيّر بمجرد التمرير لأعلى وتحميل المزيد، فلا يعكس العدد
    // الحقيقي لرسائل المحادثة أبداً. الآن نعرض العدد الحقيقي الكلي
    // (window.totalMessageCount) المحمَّل من السيرفر عبر استعلام تجميعي
    // (count) ونحدَّثه تدريجياً مع كل رسالة تُرسل/تُحذف، ونستخدم عدد
    // الرسائل المحمَّلة محلياً فقط كقيمة احتياطية مؤقتة قبل وصول العدد الحقيقي.
    const displayCount = (typeof window.totalMessageCount === 'number') ? window.totalMessageCount : window.messageCount;
    document.getElementById('onlineInfo').textContent = `${window.onlineUsers} متصل · ${displayCount} رسالة`;
...
window.blockUser = async function(username) {
    ...
    if (!confirm(`⛔ حظر ${username} بشكل دائم؟ (يمنعه من الدخول نهائياً حتى تفكّ الحظر يدوياً)`)) return;
    try {
        ...
        showNotification(`✅ تم حظر ${username} بشكل دائم`);
    } catch (error) {
...
window.unblockUser = async function(username) {
    ...
    try {
        // إصلاح: نصفّر عداد التحذيرات عند فك الحظر - وإلا لو كان الحظر جاء
        // من التصعيد التلقائي (3 تحذيرات)، يعود المستخدم بعد فك حظره وعنده
        // warnCount = 3 جاهز أصلاً فيُحظر تلقائياً مجدداً من أول تحذير واحد
        // فقط، بينما "فك الحظر" يُفترض أن يعطيه بداية جديدة فعلياً.
        await db.collection('users').doc(username).update({ blocked: false, warnCount: 0, lastWarning: null });
...
// إصلاح جوهري: كانت هذه الدالة "وهمية" بالكامل - فقط تعرض إشعار نجاح محلي
// على جهاز المسؤول نفسه، بدون أي كتابة فعلية بفايرستور. النتيجة: لا شيء
// يحدث فعلياً - لا المستخدم المُحذَّر يعرف أنه تلقى تحذيراً، ولا يوجد أي
// سجل أو عداد تحذيرات يمكن الرجوع له لاحقاً، وحتى لو أغلق المسؤول لوحة
// التحكم يختفي أي أثر للتحذير كلياً. الآن: نحفظ عدد التحذيرات ووقتها وسببها
// فعلياً بمستند المستخدم (يعرضها loadAvatar() فوراً للمستخدم المستهدف لو
// كان متصلاً حالياً، وتبقى محفوظة له لو دخل لاحقاً)، ونصعّد تلقائياً لحظر
// دائم بعد 3 تحذيرات بدل ترك المسؤول يحذّر بلا أي نتيجة حقيقية أبداً.
window.warnUser = async function(username) {
    if (!window.isAdmin) return;
    const reason = prompt(`⚠️ سبب التحذير لـ ${username} (اختياري):`, '');
    if (reason === null) return; // ألغى المسؤول العملية
    try {
        const userRef = db.collection('users').doc(username);
        const userDoc = await userRef.get();
        if (!userDoc.exists) { showNotification('❌ المستخدم غير موجود'); return; }

        const newCount = ((userDoc.data().warnCount) || 0) + 1;
        await userRef.update({
            warnCount: newCount,
            lastWarning: {
                reason: reason.trim() || 'مخالفة قوانين الدردشة',
                by: window.currentUser,
                at: firebase.firestore.FieldValue.serverTimestamp()
            }
        });
        showNotification(`⚠️ تم إرسال التحذير رقم ${newCount} إلى ${username}`);

        // تصعيد تلقائي: 3 تحذيرات = حظر دائم (نفس آلية زر "⛔ حظر" اليدوي)
        // حتى لا تبقى التحذيرات المتكررة بلا أي أثر عملي على مستخدم لا يستجيب لها.
        if (newCount >= 3) {
            await userRef.update({ blocked: true });
            await db.collection('blocked').doc(username).set({
                blockedBy: window.currentUser + ' (تلقائي بعد 3 تحذيرات)',
                blockedAt: firebase.firestore.FieldValue.serverTimestamp()
            });
            showNotification(`⛔ تم حظر ${username} تلقائياً بعد وصوله لـ 3 تحذيرات`);
        }
    } catch (error) {
        showNotification('❌ فشل إرسال التحذير');
        console.error(error);
    }
};
...
function openAdminPanel() {
    ...
    loadBadWords();
    renderAdminStickers();
    refreshUpdateCurrentBox();
    // يعيد المؤشر المنزلق فوق التبويب النشط بمجرد فتح اللوحة - يجب تأخيره
    // بإطار رسم واحد كي تُحسب أبعاد الأزرار الفعلية بعد ظهور اللوحة (display:flex)
    requestAnimationFrame(positionAdminTabIndicator);
}

// ================================================================
//  Toast خفيف لعمليات لوحة المسؤول
// ================================================================
let __adminToastTimer = null;
function showAdminToast(message) {
    const toast = document.getElementById('adminToast');
    if (!toast) { showNotification(message); return; }
    toast.textContent = message;
    toast.classList.add('show');
    clearTimeout(__adminToastTimer);
    __adminToastTimer = setTimeout(() => toast.classList.remove('show'), 2200);
}
window.showAdminToast = showAdminToast;

// ================================================================
//  مؤشر التبويبات المنزلق
// ================================================================
// يحرّك span.tab-indicator ليطابق موضع/عرض الزر النشط فعلياً (بدل
// خلفية ثابتة على كل زر)، فيعطي إحساس انتقال متصل بين التبويبات
// بدلاً من قفزة فورية بالألوان.
function positionAdminTabIndicator() {
    const wrap = document.getElementById('adminTabs');
    const indicator = document.getElementById('adminTabIndicator');
    const active = wrap ? wrap.querySelector('button.active') : null;
...
function setupSettingsModal() {
    ...
    if (versionLabel) {
        // إصلاح: كان هذا يكتب '1.0.0' ثابتاً دائماً بدل رقم الإصدار الحقيقي،
        // فيبقى المعروض بشاشة الإعدادات خاطئاً بعد كل تحديث للتطبيق حتى لو
        // كان window.APP_VERSION_NAME قد تحدّث فعلاً من PackageManager (راجع
        // syncAppVersionFromNativeBridge أعلى هذا الملف). الآن نستخدم القيمة
        // الحقيقية المتزامنة تلقائياً مع versionName بملف build.gradle.
        versionLabel.textContent = window.APP_VERSION_NAME || '1.0.0';
    }
...
function setupReactionBar() {
    document.addEventListener('contextmenu', function(e) {
        ...
        if (wrapper) {
            ...
            showReactionBar(e.clientX, e.clientY, id, text, isSent, sender);
        }
    });
}

function showReactionBar(x, y, messageId, messageText, isSent, senderName) {
    const bar = document.getElementById('reactionBar');
    const items = document.getElementById('reactionItems');
    const actionList = document.getElementById('reactionActionList');
    items.innerHTML = '';
    actionList.innerHTML = '';

    // شارة الإيموجي السريعة العلوية (نفس أسلوب إنستغرام)
    const emojis = ['❤️', '🔥', '😂', '😍', '👏', '😢'];
    emojis.forEach(emoji => {
        const span = document.createElement('span');
    ...
    function addActionRow({ icon, label, danger = false, onClick }) {
        const row = document.createElement('button');
        row.className = 'action-row' + (danger ? ' danger' : '');
        row.innerHTML = `<span class="action-icon">${icon}</span><span>${label}</span>`;
        row.onclick = () => { onClick(); bar.style.display = 'none'; };
        actionList.appendChild(row);
    }

    addActionRow({
        icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 17l-5-5 5-5"/><path d="M4 12h10a5 5 0 0 1 5 5v2"/></svg>',
        label: 'رد',
        onClick: () => startReply(messageId, messageText, senderName)
    });

    addActionRow({
        icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>',
        label: 'نسخ النص',
        onClick: () => {
            try {
                navigator.clipboard.writeText(messageText || '');
                showNotification('✅ تم النسخ');
            } catch (e) { showNotification('❌ تعذّر النسخ'); }
        }
    });

    // إصلاح: "تعديل" كانت تظهر لصاحب الرسالة أو للمسؤول معاً - أي المسؤول
    // يقدر يعدّل نص رسائل أشخاص آخرين وينتحل كلامهم. الآن تعديل النص متاح
    // فقط لصاحب الرسالة نفسه (isSent)، بينما الحذف يبقى متاحاً للمسؤول أيضاً
    // كصلاحية إشراف عادية (إزالة رسالة، لا تغيير نصها).
    if (isSent) {
        addActionRow({
            icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.83 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>',
            label: 'تعديل',
            onClick: () => window.editMessage(messageId)
        });
    }
    if (isSent || window.isAdmin) {
        addActionRow({
            icon: '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>',
            label: 'حذف',
            danger: true,
            onClick: () => window.deleteMessage(messageId)
        });
    }

    // ================================================================
    // إصلاح مهم: تموضع القائمة كان يفترض عرضاً ثابتاً (200px) وارتفاعاً
    // ثابتاً (260px) لا يطابقان الحجم الفعلي المرسوم أبداً (يتغيّر حسب عدد
    // الأسطر: "رد + نسخ" فقط لرسائل الآخرين، أو 4 أسطر لرسائلك/كمسؤول).
    // هذا الفارق كان يدفع القائمة جزئياً أو كلياً خارج حدود الشاشة (خصوصاً
    // عند الضغط المطول قرب الحواف)، فتظهر "ورا الشاشة" بشكل مقصوص أو
    // بموضع فارغ غريب. الحل: نقيس أبعادها الحقيقية بعد بنائها فعلياً (وهي
    // مخفية بصرياً بـ visibility:hidden لتفادي وميض مرئي)، ثم نموضعها
    // بحساب دقيق يضمن بقاءها كاملة داخل الشاشة دائماً مع هامش أمان ثابت،
    // ونحاول عرضها فوق نقطة الضغط أولاً (كسلوك الضغط المطول المعتاد) وإلا
    // نعرضها تحتها لو لم تتوفر مساحة كافية بالأعلى.
    // ================================================================
    ...
    left = Math.max(SAFE_MARGIN, Math.min(left, window.innerWidth - barWidth - SAFE_MARGIN));

    let top = y - barHeight - 16; // نجرّب أولاً فوق نقطة الضغط
    if (top < SAFE_MARGIN) {
        top = y + 16; // لا مساحة كافية بالأعلى - نعرضها تحت نقطة الضغط بدلاً
    }
    top = Math.max(SAFE_MARGIN, Math.min(top, window.innerHeight - barHeight - SAFE_MARGIN));
...
function setupButtons() {
    ...
    if (emojiBtn) {
        emojiBtn.onclick = function(e) {
            ...
            e.stopPropagation();
            const panel = document.getElementById('emojiPanel');
            const catTabs = document.getElementById('emojiCategoryTabs');
            const searchBox = document.getElementById('emojiSearchBox');
            if (!panel) return;
    ...
    if (emojiSearchInput) {
        emojiSearchInput.oninput = function() { filterEmojiPanel(this.value); };
    }
    ...
    // فلترة قائمة المستخدمين بلوحة المسؤول محلياً حسب اسم المستخدم وحسب
    // شريحة الحالة المختارة (الكل/متصل/مسؤولين/محظورين) معاً - كلا الفلترين
    // يعملان بالتوازي فوق نفس القائمة المعروضة أصلاً بالذاكرة (بدون طلب
    // جديد من فايرستور)، وتُعاد قراءتهما تلقائياً بعد كل تحديث حي للقائمة.
    window.__adminUserActiveFilter = window.__adminUserActiveFilter || 'all';

    function applyAdminUserFilters() {
        const q = (document.getElementById('adminUserSearch') || {}).value;
        const query = (q || '').trim().toLowerCase();
        const filter = window.__adminUserActiveFilter;
        let anyVisible = false;
        document.querySelectorAll('#adminUserList .user-item').forEach(item => {
            const nameEl = item.querySelector('.name');
            const name = nameEl ? nameEl.textContent.toLowerCase() : '';
            const matchesQuery = !query || name.includes(query);
            const matchesFilter =
                filter === 'all' ? true :
                filter === 'online' ? item.classList.contains('is-online') :
                filter === 'admin' ? item.classList.contains('is-admin') :
                filter === 'blocked' ? item.classList.contains('is-blocked') : true;
            const visible = matchesQuery && matchesFilter;
            item.style.display = visible ? '' : 'none';
            if (visible) anyVisible = true;
        });
        let emptyState = document.getElementById('adminUserEmptyState');
        const list = document.getElementById('adminUserList');
        if (!anyVisible) {
            if (!emptyState && list) {
                emptyState = document.createElement('div');
                emptyState.id = 'adminUserEmptyState';
                emptyState.className = 'admin-empty-state';
                emptyState.innerHTML = '<span class="empty-icon">🔍</span><span>لا يوجد مستخدمين مطابقين</span>';
    ...
    if (adminUserSearchEl) {
        adminUserSearchEl.oninput = applyAdminUserFilters;
    }

    document.querySelectorAll('#adminUserFilterChips button').forEach(chip => {
        chip.onclick = function() {
            document.querySelectorAll('#adminUserFilterChips button').forEach(c => c.classList.remove('active'));
            this.classList.add('active');
            window.__adminUserActiveFilter = this.dataset.filter;
    ...
    // إصلاح: الضغط على الصورة بالشريط العلوي لم يكن يفتح تعديل البروفايل
    // إلا نادراً - السبب أن المستمع onclick كان مربوطاً فقط بعنصر الصورة
    // الصغير جداً (#userAvatar, دائرة ~30px)، بينما #userAvatarWrap المحيط
    // بها (يضم الصورة + الاسم + حالة الاتصال) لم يكن له أي مستمع إطلاقاً.
    // أي ضغطة خارج تلك الدائرة الصغيرة (على الاسم مثلاً) كانت لا تفعل شيئاً،
    // فبدا وكأن فتح تعديل البروفايل يعمل فقط من القائمة الجانبية. الحل: نربط
    // كامل منطقة #userAvatarWrap لتفتح نفس نافذة تعديل البروفايل، بنفس أسلوب
    // إنستغرام حيث كل منطقة الصورة والاسم قابلة للضغط.
    const avatarWrap = document.getElementById('userAvatarWrap');
    if (avatarWrap) {
        avatarWrap.style.cursor = 'pointer';
        avatarWrap.onclick = openAvatarModal;
    } else {
    ...
    if (randomAvatarBtn) {
        randomAvatarBtn.onclick = function(e) {
            ...
            document.getElementById('avatarPreview').innerHTML =
                `<img src="${animalAvatar}" alt="avatar" /><div class="upload-overlay">📷</div>`;
        ...
        try {
            ...
            reader.onload = async function(e) {
                try {
                    ...
                    document.getElementById('avatarPreview').innerHTML =
                        `<img src="${compressed}" alt="avatar" /><div class="upload-overlay">📷</div>`;
    ...
    document.getElementById('saveEditMessageBtn').onclick = async function() {
        ...
        try {
            // نفس تحقق الملكية يتكرر هنا مباشرة قبل الكتابة الفعلية (وليس فقط
            // عند فتح النافذة بـ editMessage) - حتى لا يبقى فارق زمني يُستغل.
            // إصلاح: أُزيل استثناء المسؤول من هذا التحقق - تعديل نص الرسالة
            // مقصور على صاحبها فقط، دائماً (المسؤول يبقى قادراً على الحذف فقط).
            {
                const doc = await db.collection('messages').doc(window.editingMessageId).get();
                if (!doc.exists) { showNotification('❌ الرسالة غير موجودة'); return; }
                const data = doc.data();
                const myUid = getCurrentUid();
                const owns = data.uid ? (myUid && data.uid === myUid) : (data.username === window.currentUser);
                if (!owns) { showNotification('❌ لا يمكنك تعديل رسالة أخرى'); return; }
            }
...
async function loginExistingUser(username, { silent = false } = {}) {
    try {
        ...
        // إصلاح: حظر المسؤول اليدوي (data.blocked) لم يكن يُفحص هنا إطلاقاً -
        // فأي شخص يضغط له المسؤول زر "⛔ حظر" من لوحة التحكم كان يقدر يسجّل
        // دخول عادي جداً وكأن شيئاً لم يحدث، لأن blocked لا يُتحقق منه في أي
        // مكان بالتطبيق غير عرضه كشارة بلوحة المسؤول فقط. الآن يُمنع دخوله فعلياً.
        if (data.blocked) {
            if (!silent) {
                document.getElementById('loginError').textContent = '⛔ تم حظر حسابك من قبل المسؤول';
            }
            return false;
        }
        // إصلاح: نتحقق من انتهاء مدة الحظر المؤقت فعلياً (isBanStillActive)
        // بدل قبول/رفض الدخول بناءً فقط على data.banned، الذي كان يبقى true
        // للأبد لو فشل مؤقّت setTimeout بمتصفح المستخدم (راجع checkBadWords).
        // لو انتهت المدة فعلاً، نُلغي الحظر تلقائياً هنا (تعافٍ ذاتي) بدل
        // تعليق المستخدم بلا أي وسيلة للدخول غير تدخل المسؤول يدوياً.
        if (data.banned) {
            if (isBanStillActive(data)) {
                if (!silent) {
            ...
            try {
                await db.collection('users').doc(username).update({ banned: false, banReason: null, banExpires: null });
                data.banned = false;
            } catch (e) { /* لو فشل التحديث، سيُعاد المحاولة بأول دخول لاحق */ }
        }
...
function setupLogin() {
    ...
    document.getElementById('loginBtn').onclick = async function() {
        ...
        if (window.loginMode === 'signup') {
            // ================================================================
            // ملاحظة: قيد "حساب واحد لكل IP" اتشال نهائياً — كان يعتمد على
            // IP العام للجهاز (ipify) وهذا غير موثوق إطلاقاً على شبكات
            // الجوال (CGNAT) حيث يشترك آلاف المستخدمين المختلفين بنفس الـ
            // IP، فكان يمنع مستخدمين شرعيين كثيرين من إنشاء حسابهم الأول.
            // ================================================================
            try {
...
function enterChat(username) {
    ...
    updateMenuUserInfo();
    // إصلاح مهم: الصورة الشخصية كانت "تختفي" من الشريط العلوي (تعود للأيقونة
    // الافتراضية 👥) في كل مرة يُغلق فيها التطبيق ويُفتح من جديد، رغم أنها
    // محفوظة فعلياً بفايرستور. السبب: window.myAvatar كان يُضبط بالقيمة
    // الصحيحة قبل استدعاء enterChat (من loginExistingUser مثلاً)، لكن عنصر
    // الشريط العلوي #userAvatar بقي على القيمة الافتراضية بالـ HTML لأن
    // updateAvatarUI() كانت تُستدعى فقط من داخل مستمع loadAvatar الحي، وذاك
    // المستمع يتجاهل الرسم لو القيمة القادمة من فايرستور تطابق أصلاً
    // window.myAvatar الحالية (لا يوجد "تغيير" ليكتشفه) — فلا يحدث أي رسم
    // فعلي للعنصر عند أول دخول للشاشة. الحل: رسم الصورة فوراً هنا بغض النظر
    // عن أي مقارنة، ثم البقاء على loadAvatar لتحديثها لاحقاً لو تغيّرت فعلياً.
    updateAvatarUI();
    ...
    // ============================================================
    // نبض الحضور (presence heartbeat) + رصد الخلفية/الإغلاق
    // ============================================================
    // إصلاح: بالإضافة لتقليل الاعتماد على beforeunload وحده (راجع
    // isUserActuallyOnline أعلاه)، نضبط online:false فوراً أيضاً عند
    // visibilitychange/pagehide — وهما الحدثان اللذان يُطلقهما أندرويد
    // فعلياً عند مغادرة WebView (زر الهوم، تبديل تطبيق، تصغير)، بعكس
    // beforeunload الذي يكاد لا يُطلق إطلاقاً بتطبيقات WebView المُغلّفة.
    // وبالمقابل نعيد ضبط online:true فوراً (بدل انتظار 10 ثوانٍ) عند عودة
    // المستخدم للتطبيق حتى يظهر "متصل" بسرعة لباقي المستخدمين.
    function markOnline() {
        if (window.currentUser) {
            db.collection('users').doc(window.currentUser).update({
                online: true,
    ...
    window.presenceHeartbeat = setInterval(markOnline, 10000);
    ...
    window.addEventListener('pagehide', () => {
        clearInterval(window.presenceHeartbeat);
        markOffline();
        if (window.syncInterval) clearInterval(window.syncInterval);
        saveMessagesToStorage();
    });
    ...
    // إصلاح: كان هذا مؤجَّلاً 500ms اعتباطياً (راجع الشرح بأعلى loadMessages
    // و listenMessages) لمنع تعارض مع جلب منفصل أُزيل الآن. المستمع الحي أصبح
    // المصدر الوحيد فيُستدعى فوراً - يبدأ عرض الرسائل الحقيقية بأسرع وقت.
    listenMessages();
    initTotalMessageCount();
}
...
function loadAvatar(username) {
    db.collection('users').doc(username).onSnapshot((doc) => {
        if (doc.exists) {
            ...
            // إصلاح: حقل blocked (حظر يدوي من المسؤول) وحقل banned (حظر مؤقت
            // تلقائي من فلتر الكلمات) لم يكونا يُطبَّقان فعلياً على مستخدم
            // متصل حالياً بالدردشة - المسؤول يضغط "حظر" وتظهر له رسالة نجاح،
            // لكن الشخص المحظور يستمر بالدردشة عادي لأن ما فيه أي فحص لحظي
            // لحالته أثناء الجلسة (فقط عند تسجيل الدخول التالي). الآن أي
            // تغيير حي على هذين الحقلين يُطرد المستخدم من الجلسة فوراً.
            if ((data.blocked || (data.banned && isBanStillActive(data))) && window.currentUser === username) {
                forceLogoutDueToModeration(
                    data.blocked
                        ? '⛔ تم حظر حسابك من قبل المسؤول'
                        : `⛔ أنت محظور مؤقتاً: ${data.banReason || 'استخدام كلمات ممنوعة'}`
                );
                return;
            }

            // إصلاح: warnUser() بملف app.js كانت رسالة محلية للمسؤول فقط ولا
            // تُبلِّغ الشخص المحظور/المُحذَّر فعلياً بأي شيء. الآن نعرض
            // للمستخدم نفسه تنبيهاً حياً فور وصول تحذير جديد له (نقارن وقت
            // آخر تحذير رآه محلياً حتى لا يتكرر نفس التنبيه مع كل تحديث آخر
            // لمستنده لأي سبب غير مرتبط بالتحذير).
            if (data.lastWarning && data.lastWarning.at && window.currentUser === username) {
                const seenKey = 'tomodachi_lastWarnSeen_' + username;
                const seenMs = parseInt(localStorage.getItem(seenKey) || '0', 10);
                const warnMs = data.lastWarning.at.toMillis ? data.lastWarning.at.toMillis() : 0;
                if (warnMs && warnMs > seenMs) {
                    localStorage.setItem(seenKey, String(warnMs));
                    showNotification(`⚠️ تحذير من المسؤول: ${data.lastWarning.reason || 'يرجى الالتزام بقوانين الدردشة'}`);
                }
            }
...
// إصلاح: banExpires كانت تُحسب بـ "firebase.firestore.FieldValue.serverTimestamp() + رقم"
// بدالة checkBadWords - وهذا خطأ منطقي كامل: FieldValue.serverTimestamp() كائن خاص
// (sentinel) وليس رقماً أو وقتاً فعلياً، فجمعه مع رقم لا ينتج وقت انتهاء صحيح إطلاقاً
// (يتحول لنص مثل "[object Object]600000" أو NaN حسب المتصفح). النتيجة: banExpires
// المحفوظ غير صالح للمقارنة أبداً، وإلغاء الحظر التلقائي كان يعتمد فقط على
// setTimeout بمتصفح الشخص المحظور نفسه - لو أغلق التطبيق (متوقع لأنه للتو انحظر)
// يبقى banned:true للأبد دون أي طريقة لإلغائه غير تدخل المسؤول يدوياً. الحل: نحسب
// وقت الانتهاء الحقيقي بالمللي ثانية (Date.now()) ونتحقق منه في أي مكان نقرأ فيه
// حالة الحظر (تسجيل الدخول + الاستماع الحي)، بدل الاعتماد فقط على مؤقّت هش.
function isBanStillActive(userData) {
    if (!userData || !userData.banned) return false;
    if (!userData.banExpires) return true; // حظر بلا تاريخ انتهاء = ساري حتى يُلغى يدوياً
    const expiresMs = typeof userData.banExpires === 'number'
        ? userData.banExpires
        : (userData.banExpires.toMillis ? userData.banExpires.toMillis() : 0);
    return !expiresMs || Date.now() < expiresMs;
}

// طرد فوري من الجلسة الحالية بسبب حظر (يدوي أو تلقائي) - يسجّل خروج حقيقي من
// Firebase Auth أيضاً حتى لا يُعاد تسجيل دخوله تلقائياً بمجرد إعادة تحميل الصفحة.
function forceLogoutDueToModeration(message) {
    if (window.messagesListener) { try { window.messagesListener(); } catch (e) {} window.messagesListener = null; }
    if (window.usersListener) { try { window.usersListener(); } catch (e) {} window.usersListener = null; }
    if (window.typingListener) { try { window.typingListener(); } catch (e) {} window.typingListener = null; }
    try { window.firebaseAuth && window.firebaseAuth.signOut().catch(() => {}); } catch (e) {}
    localStorage.removeItem('lastUser');
...
// ================================================================
//  نظام التحديثات: نشر رابط APK جديد (المسؤول) + بانر تحديث (كل المستخدمين)
// ================================================================
// آلية العمل: بما إن Firebase Storage غير متاح (يحتاج خطة Blaze المدفوعة)،
// المسؤول يرفع ملف الـ APK بنفسه لأي استضافة مجانية خارجية (الأفضل: GitHub
// Releases - مجاني وبدون حدود عملية لهاد الحجم)، ثم يلصق رابط التحميل
// المباشر بتبويب "⬆️ التحديثات" بلوحة التحكم. التطبيق لا يرفع أي ملف بنفسه
// إطلاقاً - فقط يحفظ الرابط ورقم الإصدار بمستند صغير بفايرستور:
// app_meta/update_info (نص فقط، ضمن الحصة المجانية بسهولة). كل مستخدم يفتح
// التطبيق يقارن window.APP_VERSION_CODE المحلي مع versionCode المنشور؛ لو
// الأخير أكبر يظهر له بانر بأعلى الشاشة فيه رابط تحميل مباشر لملف APK الجديد.
//
// ⚠️ ملاحظة مهمة لازم تُقرأ: هذا الجزء (JS) يجهّز فقط اكتشاف وجود تحديث
// وعرض رابط تحميله. أما "تثبيت التحديث فوق النسخة القديمة بدون إلغاء
// تثبيتها" فهو سلوك نظام أندرويد نفسه عند تثبيت APK — يحدث تلقائياً بشرط:
// (1) نفس applicationId بكل نسخة، (2) versionCode أكبر من المثبت حالياً،
// (3) نفس مفتاح التوقيع (keystore) بالضبط بكل بناء. لو اختلف التوقيع بين
// نسخة وأخرى، أندرويد يرفض التثبيت ويطلب إلغاء القديمة أولاً - فتأكد أنك
// تستخدم نفس ملف الـ keystore بكل مرة تبني وتوقّع فيها APK جديد.
// كذلك: تحميل رابط .apk وفتحه تلقائياً داخل WebView يحتاج عادة كوداً
// أصلياً (DownloadListener + FileProvider) بمشروع أندرويد نفسه ليمرر
// الملف لمثبّت النظام؛ لو الرابط لا يفتح تلقائياً عند الضغط على "تحميل"،
// أضف تلك الطبقة الأصلية بمشروعك (خارج نطاق هذا الكود لأنه ملفات ويب فقط).
// ================================================================

function formatUpdateChangelog(text) {
    return (text || '')
        .split('\n')
        .map(line => line.trim())
        .filter(Boolean)
...
async function refreshUpdateCurrentBox() {
    const box = document.getElementById('updateCurrentValue');
    const deleteBtn = document.getElementById('deleteUpdateBtn');
    const copyBtn = document.getElementById('copyUpdateLinkBtn');
    if (!box) return;
    try {
        const doc = await db.collection('app_meta').doc('update_info').get();
        if (!doc.exists) {
            box.textContent = 'لا يوجد تحديث منشور بعد';
            if (deleteBtn) deleteBtn.disabled = true;
            if (copyBtn) { copyBtn.disabled = true; }
            window.__adminCurrentUpdateUrl = null;
            return;
        }
        const data = doc.data();
        const mandatoryTag = data.mandatory ? '<span class="update-mandatory-tag">⚠️ إجباري</span>' : '';
        box.innerHTML = `الإصدار ${escapeHtml(data.versionName || '?')} (كود ${escapeHtml(String(data.versionCode || '?'))})${mandatoryTag}`;
        if (deleteBtn) deleteBtn.disabled = false;
        if (copyBtn) copyBtn.disabled = !data.url;
        window.__adminCurrentUpdateUrl = data.url || null;
    } catch (e) {
        box.textContent = 'تعذّر جلب بيانات آخر إصدار';
        if (deleteBtn) deleteBtn.disabled = true;
        if (copyBtn) copyBtn.disabled = true;
    }
}

// نسخ رابط APK المنشور حالياً للحافظة بضغطة واحدة - يوفّر على المسؤول
// العودة لفايرستور يدوياً لو احتاج مشاركة الرابط بمكان آخر (مجموعة تواصل
// مثلاً) بدل الاعتماد فقط على publish الذي يخزّنه بصمت.
async function copyCurrentUpdateLink() {
    const url = window.__adminCurrentUpdateUrl;
    if (!url) return;
    try {
        ...
        showAdminToast('📋 تم نسخ رابط APK');
    } catch (e) {
        showAdminToast('❌ تعذّر النسخ');
    }
}
window.copyCurrentUpdateLink = copyCurrentUpdateLink;

// يحذف نهائياً مستند app_meta/update_info (المسؤول فقط). هذا هو الإصدار
// "المنشور حالياً" الذي تقارنه كل الأجهزة بـ APP_VERSION_CODE المحلي عندها؛
// حذفه يعني أن لا أحد سيرى إشعار توفّر تحديث بعد الآن (لا ببانر ولا بنافذة
// "حول") لحين نشر إصدار جديد لاحقاً - أي أنه فعلياً "حذف الإصدار القديم"
// الذي كان معلّقاً باللوحة كي لا يستمر بالظهور.
async function deleteCurrentUpdate() {
    if (!window.isAdmin) { showNotification('❌ فقط المسؤول يمكنه حذف التحديث'); return; }
    ...
    const confirmed = window.Swal
        ? await Swal.fire({
            title: 'حذف التحديث المنشور؟',
            text: 'لن يعود المستخدمون يرون إشعار توفّر هذا الإصدار بعد الحذف.',
            icon: 'warning',
        ...
        : confirm('حذف التحديث المنشور؟ لن يعود المستخدمون يرون إشعار توفّر هذا الإصدار بعد الحذف.');
    if (!confirmed) return;

    if (deleteBtn) { deleteBtn.disabled = true; deleteBtn.textContent = '⏳ جاري الحذف...'; }
    try {
        await db.collection('app_meta').doc('update_info').delete();
        window.__latestUpdateInfo = null;
        if (typeof setUpdateBadgeVisible === 'function') setUpdateBadgeVisible(false);
        showNotification('✅ تم حذف التحديث المنشور');
    } catch (e) {
        console.error(e);
        showNotification('❌ فشل حذف التحديث: ' + (e.message || ''));
    } finally {
        if (deleteBtn) deleteBtn.textContent = '🗑️ حذف هذا التحديث';
        refreshUpdateCurrentBox();
    }
}
window.deleteCurrentUpdate = deleteCurrentUpdate;

function setupUpdatePublish() {
    const urlInput = document.getElementById('updateApkUrl');
    const mandatorySwitch = document.getElementById('updateMandatorySwitch');
    const publishBtn = document.getElementById('publishUpdateBtn');
    const deleteBtn = document.getElementById('deleteUpdateBtn');
    const copyBtn = document.getElementById('copyUpdateLinkBtn');
    const changelogInput = document.getElementById('updateChangelog');
    const changelogPreview = document.getElementById('updateChangelogPreview');
    if (deleteBtn) deleteBtn.onclick = deleteCurrentUpdate;
    if (copyBtn) copyBtn.onclick = copyCurrentUpdateLink;

    // معاينة حيّة: كل سطر غير فارغ بحقل "ماذا الجديد؟" يتحول لنقطة منسّقة
    // فوراً، ليرى المسؤول شكل الإشعار النهائي قبل النشر لا بعده.
    if (changelogInput && changelogPreview) {
        const renderPreview = function () {
            const lines = (changelogInput.value || '')
                .split('\n')
                .map(l => l.trim().replace(/^-\s*/, ''))
                .filter(Boolean);
            changelogPreview.innerHTML = lines.length
                ? lines.map(l => `<div class="cp-item">${escapeHtml(l)}</div>`).join('')
                : '<span class="cp-empty">معاينة نقاط التحديث ستظهر هنا أثناء الكتابة...</span>';
        };
        changelogInput.oninput = renderPreview;
        renderPreview();
    }

    if (!urlInput || !publishBtn) return;

    if (mandatorySwitch) {
        mandatorySwitch.onclick = function () {
            mandatorySwitch.classList.toggle('active');
        };
    }

    publishBtn.onclick = async function () {
        if (!window.isAdmin) { showNotification('❌ فقط المسؤول يمكنه نشر تحديث'); return; }

        const versionName = document.getElementById('updateVersionName').value.trim();
        const versionCodeRaw = document.getElementById('updateVersionCode').value.trim();
        const changelog = formatUpdateChangelog(changelogInput.value);
        const mandatory = !!(mandatorySwitch && mandatorySwitch.classList.contains('active'));
        const url = urlInput.value.trim();

        if (!versionName) { showNotification('❌ أدخل رقم الإصدار'); return; }
        const versionCode = parseInt(versionCodeRaw, 10);
        if (!versionCode || versionCode < 1) { showNotification('❌ أدخل كود إصدار صحيح (رقم صحيح)'); return; }
        if (!url) { showNotification('❌ الصق رابط تحميل الـ APK'); return; }
        if (!/^https?:\/\//i.test(url)) { showNotification('❌ الرابط لازم يبدأ بـ http:// أو https://'); return; }

        publishBtn.disabled = true;
        publishBtn.classList.add('is-loading');

        try {
            await db.collection('app_meta').doc('update_info').set({
                versionName,
                versionCode,
                changelog,
                mandatory,
                url,
                publishedBy: window.currentUser,
                publishedAt: firebase.firestore.FieldValue.serverTimestamp()
            });

            showAdminToast('✅ تم نشر التحديث للجميع');
            urlInput.value = '';
            refreshUpdateCurrentBox();
        } catch (e) {
            console.error(e);
            showNotification('❌ فشل نشر التحديث: ' + (e.message || ''));
        } finally {
...
// يتحقق من وجود تحديث جديد عند دخول أي مستخدم للدردشة، ويخزّن النتيجة فقط
// (window.__latestUpdateInfo) لتسريع فتح نافذة "حول" لاحقاً. بناءً على طلب
// حصر إشعار التحديث بنافذة "حول" فقط، لا يوجد هنا أي بانر/فقاعة أو علامة
// تنبيه تظهر بأي مكان آخر بالتطبيق - openAboutModal يقوم بتحقق حقيقي إضافي
// بالخلفية عند فتحه (عبر performUpdateCheck) ليعرض الحالة الفعلية هناك فقط.
async function checkForAppUpdate() {
    try {
        const doc = await db.collection('app_meta').doc('update_info').get();
        window.__latestUpdateInfo = doc.exists ? doc.data() : null;
    } catch (e) {
        console.warn('تعذّر التحقق من وجود تحديث:', e);
    }
}

// ================================================================
//  مودال «حول» + الفحص الفعلي للتحديثات من القائمة الجانبية
// ================================================================
// يربط عنصر "حول" بالقائمة الجانبية (جنب صورة المستخدم) بنافذة معلومات
// عن التطبيق والمطور slman، ويسمح بالتحقق يدوياً من وجود تحديث في أي وقت
// - على نفس مستند app_meta/update_info المستخدم فعلياً بباقي التطبيق (لا
// يوجد أي منطق تحديث مزدوج أو وهمي). كما تظهر علامة "*" حمراء بجانب
// "حول" بالقائمة نفسها متى توفّر تحديث فعلي (window.__latestUpdateInfo
// يُحدَّث من checkForAppUpdate تلقائياً عند الدخول، وأيضاً من هنا عند أي
// فحص يدوي)، فيبقى المستخدم على علم حتى لو أغلق بانر التحديث العلوي.
// ================================================================

// ملاحظة: لا يوجد أي علامة/فقاعة تنبيه تعرض هذه الحالة خارج نافذة "حول" -
// الدالة تحتفظ فقط بحالة داخلية (window.__updateAvailable) يستخدمها كود
// النافذة نفسها (aboutUpdateStatusRow/Dot) عند فتحها.
function setUpdateBadgeVisible(visible) {
    window.__updateAvailable = !!visible;
}
window.setUpdateBadgeVisible = setUpdateBadgeVisible;

async function performUpdateCheck({ silent = false } = {}) {
    const statusLabel = document.getElementById('aboutUpdateStatusLabel');
    const statusSub = document.getElementById('aboutUpdateStatusSub');
    const statusRow = document.getElementById('aboutUpdateStatusRow');
    const statusIcon = document.getElementById('aboutUpdateStatusIcon');
    const changelogBox = document.getElementById('aboutUpdateChangelogBox');
    const actionBtn = document.getElementById('aboutCheckUpdateBtn');

    function setCardState(state) {
        if (!statusRow) return;
        statusRow.classList.remove('state-checking', 'state-uptodate', 'state-available', 'state-error');
        statusRow.classList.add('state-' + state);
    }

    if (!silent) {
        setCardState('checking');
        if (statusIcon) statusIcon.textContent = '⏳';
        if (statusLabel) statusLabel.textContent = 'جاري التحقق...';
        if (statusSub) statusSub.textContent = 'يتحقق من وجود إصدار أحدث';
        if (actionBtn) { actionBtn.disabled = true; actionBtn.classList.add('checking'); actionBtn.textContent = '⏳ جاري التحقق...'; }
    }

    try {
        const doc = await db.collection('app_meta').doc('update_info').get();
        const data = doc.exists ? doc.data() : null;
        window.__latestUpdateInfo = data;
        const remoteCode = data ? (data.versionCode || 0) : 0;
        const hasUpdate = !!data && remoteCode > window.APP_VERSION_CODE;

        setUpdateBadgeVisible(hasUpdate);

        if (!data) {
            setCardState('uptodate');
            if (statusIcon) statusIcon.textContent = '✅';
            if (statusLabel) statusLabel.textContent = 'أنت تستخدم أحدث إصدار متوفر';
            if (statusSub) statusSub.textContent = 'لا يوجد أي إصدار منشور بعد';
        } else if (hasUpdate) {
            setCardState('available');
            if (statusIcon) statusIcon.textContent = '🎉';
            if (statusLabel) statusLabel.textContent = `يتوفر إصدار جديد: ${data.versionName || remoteCode}`;
            if (statusSub) statusSub.textContent = data.mandatory ? '⚠️ هذا التحديث إجباري' : 'يُنصح بالتحديث للحصول على أحدث الميزات';
        } else {
            setCardState('uptodate');
            if (statusIcon) statusIcon.textContent = '✅';
            if (statusLabel) statusLabel.textContent = 'أنت تستخدم أحدث إصدار متوفر';
            if (statusSub) statusSub.textContent = `الإصدار الحالي: ${window.APP_VERSION_NAME || '1.0.0'}`;
        }

        if (changelogBox) {
            if (hasUpdate && data.changelog) {
                const lines = String(data.changelog).split('\n').map(l => l.trim()).filter(Boolean);
                changelogBox.innerHTML = lines.map(l => `<div class="cp-item">${escapeHtml(l)}</div>`).join('');
        ...
        if (actionBtn) {
            ...
            actionBtn.classList.remove('checking');
            if (hasUpdate) {
                actionBtn.classList.add('state-available-btn');
                actionBtn.textContent = '⬇️ تحميل آخر إصدار';
                actionBtn.onclick = function () {
                    if (window.AndroidBridge && typeof window.AndroidBridge.downloadUpdate === 'function') {
                        window.AndroidBridge.downloadUpdate(data.url);
                    } else {
                        window.open(data.url, '_blank');
                    }
                };
            } else {
                actionBtn.classList.remove('state-available-btn');
                actionBtn.textContent = '🔄 تحقق من التحديثات';
                actionBtn.onclick = () => performUpdateCheck();
            }
        }

        return hasUpdate;
    } catch (e) {
        console.warn('تعذّر التحقق من التحديثات:', e);
        setCardState('error');
        if (statusIcon) statusIcon.textContent = '⚠️';
        if (statusLabel) statusLabel.textContent = 'تعذّر الاتصال';
        if (statusSub) statusSub.textContent = 'تحقق من الإنترنت وحاول مجدداً';
        if (actionBtn) {
            actionBtn.disabled = false;
            actionBtn.classList.remove('checking', 'state-available-btn');
            actionBtn.textContent = '🔄 إعادة المحاولة';
            actionBtn.onclick = () => performUpdateCheck();
        }
        return window.__updateAvailable || false;
    }
}
window.performUpdateCheck = performUpdateCheck;

function openAboutModal() {
    const modal = document.getElementById('aboutModal');
    if (!modal) return;

    const versionEl = document.getElementById('aboutCurrentVersion');
    if (versionEl) versionEl.textContent = `الإصدار ${window.APP_VERSION_NAME || '1.0.0'} (كود ${window.APP_VERSION_CODE})`;
    ...
    // نعرض فوراً آخر نتيجة معروفة (بلا وميض/انتظار) لو متوفرة، ثم نتحقق
    // فعلياً بالخلفية للتأكد أنها ما زالت صحيحة (بدون حجب الواجهة بانتظارها)
    performUpdateCheck({ silent: window.__latestUpdateInfo !== undefined });
}
...
function setupAboutModal() {
    ...
    const closeBtn2 = document.getElementById('closeAboutModal2');
    [closeBtn, closeBtn2].forEach(btn => { if (btn) btn.onclick = closeAboutModal; });
    modal.onclick = function (e) { if (e.target === this) closeAboutModal(); };

    const actionBtn = document.getElementById('aboutCheckUpdateBtn');
    if (actionBtn) actionBtn.onclick = () => performUpdateCheck();
}
