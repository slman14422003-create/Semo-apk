...
function listenUsers() {
    ...
    window.usersListener = db.collection('users').onSnapshot((snapshot) => {
        ...
        if (window.isAdmin) {
            ...
            // تعيد تطبيق فلتر البحث الحالي (إن وجد) بعد كل تحديث حي للقائمة،
            // وإلا يُفقَد الفلتر مع أول تحديث حالة اتصال قادم من فايرستور.
            const searchBox = document.getElementById('adminUserSearch');
...
function setupButtons() {
    ...
    // فلترة قائمة المستخدمين بلوحة المسؤول محلياً حسب اسم المستخدم - تسهّل
    // التحكم بالضبط اللي طلبه المسؤول لما عدد الأعضاء يكبر (بدل التمرير
    // اليدوي بينهم كلهم للوصول لعضو واحد).
    const adminUserSearchEl = document.getElementById('adminUserSearch');
    if (adminUserSearchEl) {
        adminUserSearchEl.oninput = function() {
...
async function refreshUpdateCurrentBox() {
    ...
    try {
        ...
        const data = doc.data();
        box.textContent = `الإصدار ${data.versionName || '?'} (كود ${data.versionCode || '?'})` +
            (data.mandatory ? ' — إجباري' : '');
...
function setupUpdatePublish() {
    ...
    publishBtn.onclick = async function () {
        ...
        const changelog = formatUpdateChangelog(document.getElementById('updateChangelog').value);
        ...
        try {
            ...
            showNotification('✅ تم نشر التحديث للجميع');
            urlInput.value = '';
            refreshUpdateCurrentBox();
        } catch (e) {
            console.error(e);
...
async function performUpdateCheck({ silent = false } = {}) {
    ...
    const statusDot = document.getElementById('aboutUpdateStatusDot');
    ...
    if (!silent) {
        if (statusLabel) statusLabel.textContent = '⏳ جاري التحقق من التحديثات...';
    ...
    try {
        ...
        if (statusLabel) {
            if (!data) {
                statusLabel.textContent = 'لا يوجد أي إصدار منشور بعد';
            } else if (hasUpdate) {
                statusLabel.textContent = `🎉 يتوفر إصدار جديد: ${data.versionName || remoteCode}`;
            } else {
                statusLabel.textContent = '✅ أنت تستخدم أحدث إصدار متوفر';
            }
        }
        if (statusRow) statusRow.classList.toggle('has-update', hasUpdate);
        if (statusDot) statusDot.style.display = hasUpdate ? 'inline' : 'none';

        if (changelogBox) {
            if (hasUpdate && data.changelog) {
                changelogBox.textContent = data.changelog;
        ...
        if (statusLabel) statusLabel.textContent = '⚠️ تعذّر الاتصال - تحقق من الإنترنت وحاول مجدداً';
        if (statusRow) statusRow.classList.remove('has-update');
        if (actionBtn) {
            actionBtn.disabled = false;
            actionBtn.classList.remove('checking');
...
function openAboutModal() {
    ...
    if (versionEl) versionEl.textContent = `${window.APP_VERSION_NAME || '1.0.0'} (كود ${window.APP_VERSION_CODE})`;
